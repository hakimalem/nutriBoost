...
export const RateProductButton = () => {
    const [ratingInput, setRatingInput] = React.useState(0);
    const [showModal, setShowModal] = React.useState(false);

    return (
        <>
            <button onClick={() => setShowModal(true)} className=" text-lg py-[3%] my-[8%] w-[90%] bg-slate-400  rounded-md hover:bg-yellow-400  active:bg-yellow-200 text-white font-poppinsBold shadow hover:shadow-lg outline-none focus:outline-none ease-linear transition-all duration-150">
                Write Review
            </button>
            {showModal ? (
                <>
                            ...
                            <div className=" border-0 rounded-xl w-[100%] shadow-lg relative flex flex-col bg-white outline-none focus:outline-none">
                                ...
                                <div className="flex flex-col py-[5%] items-center justify-center ">
                                    ...
                                    <div className="flex flex-row justify-center items-center gap-4 text-xl font-thin text-text-black w-[50%]">
                                        <img src={profile.image} alt="" className=" rounded-full w-20 h-20" />
                                        <div>
                                            <h1>{profile.name}</h1>
                                    ...
                                    <div className="flex flex-row justify-between w-[70%] mb-[4%]">
                                        ...
                                        <FormControl fullWidth sx={{ m: 1 }} variant="standard" className="flex flex-row justify-between w-[70%]">
                                            <TextField
                                    ...
                                    <div className="w-[68%] gap-[7%] flex justify-between mb-[4%]">
                                        <button onClick={() => setShowModal(false)} className='text-lg py-[1.5%] w-[50%] border border-text-gray bg-dragos-input text-text-gray2 font-normal rounded-md hover:bg-text-gray hover:text-white  active:bg-white shadow hover:shadow-lg outline-none focus:outline-none ease-linear transit transition-all duration-50'>
                                            Cancel
                                        </button>
                                        <button onClick={() => setShowModal(true)} className='text-xl py-[1%] w-[50%] bg-primary text-white font-normal rounded-md hover:bg-green-400  active:bg-white active:text-text-gray shadow hover:shadow-lg outline-none focus:outline-none ease-linear transit transition-all duration-50'>
                                            Submit Review
                                        </button>
                                    </div>
                                </div>

...
function ReviewProgressBar(props) {
    ...
    return (
        <div className="flex items-center my-[2%]">
            ...
            <div className=" w-[85%]">
                <BorderLinearProgress variant="determinate" value={nbReviews * 100 / totalReviews} className="w-[100%]" />
...
export const ReviewPreview = () => {
    ...
    const averageReviews = (5 * reviews[0].nbReviews + 4 * reviews[1].nbReviews + 3 * reviews[2].nbReviews + 2 * reviews[3].nbReviews + reviews[4].nbReviews) / totalReviews;
    return (
        <div className="bg-card-gray border-card-stroke rounded-md w-[25%] flex flex-col justify-between p-[1%] items-center mr-[1%]" >
            <div className="flex flex-col justify-between items-center my-[8%]">
                <h1 className="text-text-black text-5xl font-normal mb-[8%]">{averageReviews.toFixed(1)}</h1>
                <Rating name="half-rating" defaultValue={averageReviews} precision={0.2} size="large" readOnly className="mb-[5%]" />
                <h1 className="">Overall {totalReviews} Review</h1>
            </div>
            <div className="bg-white flex flex-col rounded-md w-[90%] p-[5%]">
                {reviews.map(review => (
                    <ReviewProgressBar key={review.category} category={review.category} nbReviews={review.nbReviews} totalReviews={totalReviews} />
                ))}
            </div>
            <RateProductButton />
